      ...
      {moods.map((mood) => {
        ...
        return (
          <button
            ...
            className={`flex shrink-0 flex-col items-center rounded-2xl border transition-all duration-200 ${compact ? 'min-w-[3rem] px-3 py-2.5' : 'px-3 py-2.5'} ${active ? 'border-border bg-inset-3 scale-105' : 'border-border bg-inset'}`}
          >
            <span className={compact ? 'text-2xl leading-none' : 'text-xl'}>{cfg.emoji}</span>
